...
#
# $Id: compile.sh 3026 2009-12-05 16:25:51Z eldering $
...
logmsg $LOG_DEBUG "$RUNGUARD ${DEBUG:+-v} -t $COMPILETIME -f $FILELIMIT -o $WORKDIR/compile.time $COMPILE_SCRIPT `basename $SOURCE` source $MEMLIMIT &>$WORKDIR/compile.tmp"
...
if [ -f source ]; then
    logmsg $LOG_DEBUG "mv -f source program"
    mv -f source program
    chmod a+rx program
fi
...
logmsg $LOG_DEBUG "cat compile.tmp >>compile.out"
cat compile.tmp >>compile.out
