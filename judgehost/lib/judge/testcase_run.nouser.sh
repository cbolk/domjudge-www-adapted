...
logmsg $LOG_INFO "$RUNGUARD ${DEBUG:+-v} ${USE_CHROOT:+-r "$PWD"} -u $RUNUSER -t $TIMELIMIT -m $MEMLIMIT -f $FILELIMIT -p $PROCLIMIT -c -o program.time -- $PREFIX/$RUN_SCRIPT $PREFIX/program testdata.in program.out program.err program.exit  &>error.tmp "

runcheck "$RUNGUARD" ${DEBUG:+-v} ${USE_CHROOT:+-r "$PWD"}  \
