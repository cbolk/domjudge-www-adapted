...
$source = $DB->q('MAYBETUPLE SELECT sourcecode, probid, langid, submittime FROM submission
                  LEFT JOIN language USING(langid)
                  WHERE submitid = %i',$id);
if ( empty($source) ) error ("Submission $id not found");
$oldsource = $DB->q('MAYBETUPLE SELECT sourcecode, submission.langid FROM submission
                     LEFT JOIN language USING(langid)
