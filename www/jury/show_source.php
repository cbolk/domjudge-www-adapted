...
if ( strlen($lastcode)==0 ) {
...
} elseif ( strlen($lastcode) < 10 * 1024 ) {
	...
	echo htmlspecialchars($lastcode) ;
	echo '</pre>';
} else {
...
if ( $oldsource ) {
	...
	if(class_exists('Tiny_diff')){
		$tiny_diff = new Tiny_diff();
		$newcode = htmlspecialchars($source['sourcecode']);
		$oldcode = htmlspecialchars($oldsource['sourcecode']);
		$difference = $tiny_diff->compare($oldcode, $newcode, 'normal');
		$difference = htmlspecialchars($difference);
		
	} elseif ( function_exists('xdiff_string_diff') ) {
		// The PECL xdiff PHP-extension.

		$difftext = xdiff_string_diff($oldcode,
		                              $lastcode,2);
		...
		if ( is_readable($oldfile) && is_readable($newfile) ) {
			...
			if( ! $oldfile || ! $newfile ) {
				...
				if( ! $oldhandle || ! $newhandle ) {
					...
					if ( (fwrite($oldhandle,$oldcode)===FALSE) ||
					     (fwrite($newhandle,$lastcode)===FALSE) ) {
	...
	echo '<pre class="output_text">' . parseDiff($difference) . "</pre>\n\n";
} else {
	echo "<hr>&lt; <a href='submission.php?id=$id'>back</a>";
}
