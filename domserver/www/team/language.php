...
?>
<table>
<tr><td class="theader" scope="row">ID:          </td><td><?php echo htmlspecialchars($data['langid'])?></td></tr>
<tr><td class="theader" scope="row">Name:        </td><td><?php echo htmlspecialchars($data['name'])?></td></tr>
<tr><td class="theader" scope="row">Extension:   </td><td class="filename">.<?php echo htmlspecialchars($data['extension'])?></td></tr>
<?php
$languageicon = strtolower('../images/languages/' . $data['name'] . '_file.png');
if ( is_readable($languageicon) ) {
	echo '<tr><td class="theader" scope="row">Icon:   </td><td class="fileicon"><img class="imgmiddle" src="' . $languageicon . '"' .
		 ' alt="'   . htmlspecialchars($submdata['langname']) . '"' .
		 ' title="' . htmlspecialchars($submdata['langname']) . '" /></td></tr>';
}
?>
<tr><td class="theader" scope="row">Allow submit:</td><td><?php echo printyn($data['allow_submit']) . ' '.
	addSubmit('toggle', 'cmd[toggle_submit]',
		"return confirm('" . ($data['allow_submit'] ? 'Disallow' : 'Allow') .
		" submissions for this language?')"); ?>
</td></tr>
<tr><td class="theader" scope="row">Allow judge: </td><td><?php echo printyn($data['allow_judge']) . ' ' .
	addSubmit('toggle', 'cmd[toggle_judge]',
		"return confirm('" . ($data['allow_judge'] ? 'Disallow' : 'Allow') .
		" judging for this language?')"); ?>
</td></tr>
<tr><td class="theader" scope="row">Time factor:  </td><td><?php echo htmlspecialchars($data['time_factor'])?> x</td></tr>
</table>
