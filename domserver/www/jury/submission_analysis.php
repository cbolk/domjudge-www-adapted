...
$submdata = $DB->q('MAYBETUPLE SELECT s.teamid, s.probid, s.langid, sf.sourcecode,
                    ...
                    FROM submission s
					LEFT JOIN submission_file sf ON (s.submitid = sf.submitid)
                    LEFT JOIN team     t ON (t.login  = s.teamid)
                    LEFT JOIN problem  p ON (p.probid = s.probid)
                    LEFT JOIN language l ON (l.langid = s.langid)
                    LEFT JOIN contest  c ON (c.cid    = s.cid)
                    WHERE s.submitid = %i', $id);
...
<tr><td scope="row">Source:</td><td>
	<a href="#sourcecode">view source code</a></td></tr>
...
<?php
	...
	/* source code */
	echo "<h3 name=\"sourcecode\">Source code</h3>\n";
	echo "<pre class=\"brush: c\">\n";
	echo htmlspecialchars($srccode);
	echo "\n</pre>";
