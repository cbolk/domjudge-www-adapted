...
$probs = $DB->q("COLUMN SELECT p.probid FROM problem p INNER JOIN contest c ON p.cid = c.cid
                 WHERE c.contestname LIKE 'Consegna%%' ORDER BY p.cid, p.probid");
...
foreach( $teams as $team ) {
	...
	foreach( $probs as $pr ) {
		...
		calcHomeworkStats($team['login'], $pr);
	}
...
echo "<p>Homework cache refresh completed in ".round($time_end - $time_start,2)." seconds.</p>\n\n";
