...
$id = (int)$_GET['id'];
$submission = $DB->q('MAYBETUPLE SELECT * FROM submission
                  LEFT JOIN language USING(langid)
                  WHERE submitid = %i',$id);

if ( empty($submission) ) error ("Submission $id not found");

$sourcefile = getSourceFilename($submission['cid'],$id,$submission['teamid'],
                                $submission['probid'],$submission['extension']);

$title = 'Edit Source: ' . htmlspecialchars($sourcefile);
