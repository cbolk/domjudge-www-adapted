...
require('init.php');
$title = 'Team Participation @ Homework';

require(LIBWWWDIR . '/header.php');

echo "<h1>Teams @ HOME</h1>\n\n";

$strSQL = "SELECT teamid, t.name, probid, submissions, is_correct FROM homework sbj LEFT JOIN team t ON (sbj.teamid = t.login) INNER JOIN contest ON sbj.cid = contest.cid where contestname like 'Consegna%%' and t.`categoryid` = 1 ORDER BY teamid, probid;";
$res = $DB->q($strSQL);

$strSQL = 'SELECT DISTINCT sbj.probid, name, problem.cid FROM homework sbj INNER JOIN problem ON sbj.probid = problem.probid INNER JOIN contest c on problem.cid = c.cid where contestname like \'Consegna%%\' ORDER BY problem.cid, c.starttime, sbj.probid';
$probs = $DB->q($strSQL);
$nprob = $probs->count();

$strSQL = 'SELECT contest.cid, contest.contestname, COUNT(probid) AS nprob_cid FROM contest INNER JOIN problem on contest.cid = problem.cid  where contestname like \'Consegna%%\'  group by contest.cid order by cid;';
$conts = $DB->q($strSQL);
$nconts = $conts->count();
