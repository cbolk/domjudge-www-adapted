...
function putSubmissions($cdata, $restrictions, $limit = 0)
{
	...
	echo "<table class=\"list sortable\">\n<tr>" .
		...
		(IS_JURY ? "<th scope='col' class='theader'>ID</th>" : '') .
		"<th scope='col' class='theader'>time</th>" .
		(IS_JURY ? "<th scope='col' class='theader'>team</th>" : '') .
		"<th scope='col' class='theader'>problem</th>" .
	...
	while( $row = $res->next() ) {
		...
		if ( is_readable($languageicon) ) {
			echo '<img class="imgmiddle" src="' . $languageicon . '"' .
				 ...
				 ' title="' . htmlspecialchars($row['langname']) . '" />';
		} else {
...
function putClock() {
	...
	// current time
	echo '<div id="clock">';
	echo '<div class="clock-date" >' . '<strong>'.  strftime('%e') . '</strong><em>' . strftime('%b %Y') .'</em>'; 
	...
	if ( strcmp(now(), $cdata['starttime']) >= 0 && strcmp(now(), $cdata['endtime']) < 0) {
		...
		echo "<div id='timeleft'><img class='smallpic' style='padding: 0px;' src='../images/timeleft.png' alt='time left' width='20px'/><span> " . $fmt . "</span></div>";
	}
	echo "</div>";
	echo "</div>\n\n";
}
